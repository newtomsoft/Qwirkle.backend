...
public static class ComputePoints
{
    ...
    public static int Compute(Game game, List<TileOnBoard> tiles)
    {
        ...
        if (tiles.Count(t => t.Coordinates.Y == tiles[0].Coordinates.Y) == tiles.Count)
        {
            ...
            if (tiles.Count > 1)
            {
                ...
                {
                    if ((points = ComputePointsInColumn(new List<TileOnBoard> { tile }, game)) is 0) return 0;
                    if (points is not 1) totalPoints += points;
                }
            }
        }
        ...
        foreach (var tile in tiles)
        {
            if ((points = ComputePointsInLine(new List<TileOnBoard> { tile }, game)) == 0) return 0;
            if (points != 1) totalPoints += points;
        }
        return totalPoints;
    }

    private static int ComputePointsInLine(IReadOnlyList<TileOnBoard> tiles, Game game)
    {
    ...
    private static int ComputePointsInColumn(IReadOnlyList<TileOnBoard> tiles, Game game)
    {
        var allTilesAlongReferenceTiles = tiles.ToList();
